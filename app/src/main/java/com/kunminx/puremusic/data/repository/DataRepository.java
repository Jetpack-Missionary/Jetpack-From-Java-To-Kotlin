...
public class DataRepository implements ILocalRequest, IRemoteRequest {
    ...
    public void getJavaItems(MutableLiveData<List<GridItem>> liveData) {
        ...
        for (int i = 0; i < 10; i++) {
            GridItem gridItem = new GridItem("组件", R.drawable.bg_album_default);
            list.add(gridItem);
        }
...
*/
        ...
        Type type = new TypeToken<List<GridItem>>() {
        }.getType();
        List<GridItem> testAlbum = gson.fromJson(Utils.getApp().getString(R.string.free_music_json), type);
